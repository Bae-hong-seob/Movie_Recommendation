...
class VAE(nn.Module):
    ...
    def reparameterize(self, h):
        ...
        make mu, logvar using h(vector) linear layer
            Args: 
                h : encoder last output
                    vector(hidden dim)
            
            Return:
                z : mu + (std*eps) has same dims with encoder output and decoder input
        '''
        mu, logvar = nn.Linear(self.encoder_dims[-1], self.encoder_dims[-1]//2)(h), nn.Linear(self.encoder_dims[-1], self.encoder_dims[-1]//2)(h)
        std = torch.exp(0.5 * logvar)
        eps = torch.randn_like(std)
        
        z = mu + (std*eps)
        z = nn.Linear(self.encoder_dims[-1]//2, self.encoder_dims[-1])(z)
        return z
